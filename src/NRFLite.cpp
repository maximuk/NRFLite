...
uint8_t NRFLite::initRadio(uint16_t radioId, Bitrates bitrate, uint8_t channel)
{
    ...
    uint8_t address[5] = {(uint8_t)radioId, (uint8_t)(radioId >> 8), ADDRESS_PREFIX[2], ADDRESS_PREFIX[1], ADDRESS_PREFIX[0]};
...
void NRFLite::prepForTx(uint16_t toRadioId, SendType sendType)
{
    if (_lastToRadioId != toRadioId)
    {
        ...
        uint8_t address[5] = {(uint8_t)toRadioId, (uint8_t)(toRadioId >> 8), ADDRESS_PREFIX[2], ADDRESS_PREFIX[1], ADDRESS_PREFIX[0]};
