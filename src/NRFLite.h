...
public:
    ...
    uint8_t init(uint16_t radioId, uint8_t cePin, uint8_t csnPin, Bitrates bitrate = BITRATE2MBPS, uint8_t channel = 100, uint8_t callSpiBegin = 1);
#if defined(__AVR__)
    uint8_t initTwoPin(uint16_t radioId, uint8_t momiPin, uint8_t sckPin, Bitrates bitrate = BITRATE2MBPS, uint8_t channel = 100);
#endif
    ...
    uint8_t send(uint16_t toRadioId, void *data, uint8_t length, SendType sendType = REQUIRE_ACK);
    uint8_t hasAckData();
    ...
    uint8_t hasDataISR();
    uint8_t startRx();
    void startSend(uint16_t toRadioId, void *data, uint8_t length, SendType sendType = REQUIRE_ACK);
    void whatHappened(uint8_t &txOk, uint8_t &txFail, uint8_t &rxReady);

private:
    constexpr static uint8_t ADDRESS_PREFIX[3] = {5, 4, 3}; // 1st 3 bytes of addresses, 4th and 5th bytes will be RadioId.
    ...
    int32_t _lastToRadioId = -1;
    uint32_t _microsSinceLastDataCheck;

    uint8_t getPipeOfFirstRxPacket();
    uint8_t getRxPacketLength();
    uint8_t initRadio(uint16_t radioId, Bitrates bitrate, uint8_t channel);
    void prepForTx(uint16_t toRadioId, SendType sendType);
